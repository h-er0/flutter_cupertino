...
class LiquidButtonFactory {
    static func create(with args: [String: Any]) -> UIView {
        ...
        if useLiquid {
             setupLiquidGlass(view: container, args: args)
        } else {
             setupLegacy(view: container, args: args)
        }
        
        return container
    }
    
    private static func setupLiquidGlass(view: UIView, args: [String: Any]) {
        // Liquid Glass styling: Blur, translucency, rounded corners
        view.backgroundColor = .clear
        
        // Ensure bounds are set later or constraints are used, but for now we operate on the view
        // Note: For AutoLayout, we shouldn't rely on bounds == 0.
        // We will add the blur view and pin it.
    ...
    private static func setupLegacy(view: UIView, args: [String: Any]) {
    ...
    private static func addContent(to view: UIView, args: [String: Any]) {
        ...
        stackView.isUserInteractionEnabled = false // Let touches pass through to the button/container
    ...
    private static func uiColor(from arg: Int64) -> UIColor {
